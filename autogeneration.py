###user to edit script here
Preparer = '' #e.g. 'Mark Joshua T. Ramos' - quotation marks included
Reviewer = '' #e.g. 'Mark Joshua T. Ramos' - quotation marks included
output_path = r'' #(note the backslash)change this link to the directory of the memo - do not remove the r in the beginning  e.g. r'C:\Users\GA768JR\OneDrive - EY\Desktop\ Auto Memo'
fundRequest = '' #e.g. 'fundRequest.csv'
row_numbers = []  #retain the brackets. change this to the row number of the file in the FR - excluding the headers (NOTE: 0 is the first item)
                  #e.g. [0] or [0,1,2] for multiple lines or ['All'] for all the funds inside the fund request
...
#validation - if the output path is not found
try:
    os.chdir(output_path)
except:
    print('Invalid Run: Folder specified under variable output_path was not found. Please ensure that the path is correct.')
    invalidCount += 1

#validation - if fund request is not found in the provided link
try:
    doc_fr = pd.read_csv(fundRequest) #change this to the to the fund request filename that should be in the same folder of the directory of the memo - do not drop the r in the beginning
except:
    print('Invalid Run: Fund Request not found. Please check if the filename provided for doc_fr is correct or the output_path is valid.')
    invalidCount += 1
    
#validation - if the datatype of row_numbers is not list
if type(row_numbers)!=list:
    print("Invalid Run: The variable row_number contains invalid values. Please make sure that its datatype is list e.g. [0] or [0,1,2] for multiple lines or ['All'] for all the funds inside the fund request.")
    invalidCount += 1
else:
    pass

#validation - if row_numbers contains more rows than the fund request provided
try:
    if len(row_numbers) > len(doc_fr):
        print("Invalid Run: The row numbers provided in row_numbers variable is greater than the row count inside the Fund Request file. Please make sure to only provide the row numbers found in the Fund Request file into row_numbers variable.")
        invalidCount += 1
    else:
        pass
except:
...
#validation - if row_numbers contains non-integers if it not 'All'
typeCount = 0
if row_numbers != ['All']:
    res_list = []
    
    for item in row_numbers: 
        if type(item) not in res_list: 
            res_list.append(type(item)) 
...
if typeCount != 1:
    print("Invalid Run: Please check if the content of row_numbers variable follows any of the sample e.g. [0] or [0,1,2] for multiple lines or ['All'] for all the funds inside the fund request. Do note that the contents should only be 'All' or integers-only.")
    invalidCount += 1
else:
    pass

#validation - if max number provided by the user is greater than the amount of rows in the FR
try:
    if max(row_numbers) > len(doc_fr)-1:
        print("Invalid Run: The highest number provided in row_numbers variable is greater than the number of rows in the FR hence an invalid run")
        invalidCount += 1
    else:
        pass
except:
...
if invalidCount > 0:
    print('Running Completed: Please fix all the issues noted above then rerun to proceed.')
else:
    runNum = 0
            ...
            for (value,names,workpaper,newcol,newcol_reclass,sheets,tab,reco,reclass_tab,reco_pic) in inputs:
                if abs(value)>1:
                    ...
                    if tab not in sheets.sheet_names:
                        reco.append('The difference in %s is either due to the missing %s input file or incorrect account mapping. As such audit team shall obtain the missing file or modify the account mapping and ask  team for a refreshed output.'%(workpaper,workpaper))  
    ...
    #         sec2a.text = (memo_text)
        ...
        #doc.tables[0].cell(5,1).text =currentDate.strftime('%d %B %Y')
        ...
        ### set variables of the  source files to be used
        #set file
        wp_path = '/Workpapers'
        files = os.listdir(output_path+'/'+wp_path)
        os.chdir(output_path+'/'+wp_path)

        _Leads = [file for file in files if "%s_%s_Lead_Sheets" %(str(doc_fr['FundShortCode'][row_num])[0:10],fileDate) in file]
        _CapAct = [file for file in files if "%s_%s_Capital_Activity" %(str(doc_fr['FundShortCode'][row_num])[0:10],fileDate) in file]
        _CashRecon = [file for file in files if "%s_%s_Cash_Reconciliation" %(str(doc_fr['FundShortCode'][row_num])[0:10],fileDate) in file]
        _Dividends = [file for file in files if "%s_%s_Dividends" %(str(doc_fr['FundShortCode'][row_num])[0:10],fileDate) in file]
        _Interest = [file for file in files if "%s_%s_Interest" %(str(doc_fr['FundShortCode'][row_num])[0:10],fileDate) in file]
        _InvRF = [file for file in files if "%s_%s_Investment_Roll" %(str(doc_fr['FundShortCode'][row_num])[0:10],fileDate) in file]
        _PortVal = [file for file in files if "%s_%s_Portfolio_Val" %(str(doc_fr['FundShortCode'][row_num])[0:10],fileDate) in file]
        _RGL = [file for file in files if "%s_%s_Realized_Gain" %(str(doc_fr['FundShortCode'][row_num])[0:10],fileDate) in file]
        _Trades = [file for file in files if "%s_%s_Unsettled_Trades" %(str(doc_fr['FundShortCode'][row_num])[0:10],fileDate) in file]
        _FXReason = [file for file in files if "%s_%s_FX_Reasonability" %(str(doc_fr['FundShortCode'][row_num])[0:10],fileDate) in file]
        _NAVbased = [file for file in files if "%s_%s_NAV_Based" %(str(doc_fr['FundShortCode'][row_num])[0:10],fileDate) in file]
        
        #`validation - if fund code and end date did not match any workpapers
        if _Leads == _CapAct == _CashRecon == _Dividends == _Interest == _InvRF == \
        _PortVal == _RGL == _Trades == _FXReason == _NAVbased == []:
            print("(FR Row Number " + str(row_num) + ") Invalid Run: The specified fund code and financial end date in the FR did not match any of the workpapers in the filepath.")
            ...
            if len(_Leads)!=0:
                ...
                workbook = xl.Workbooks.Open(output_path+'/'+wp_path+'/'+_Leads[0])
                workbook.Save()

                #trial balance CY & PY test
                try:
                    Leads_TB = pd.read_excel(output_path+'/'+wp_path+'/'+_Leads[0], sheet_name='A_Trial_Balance', header=header_no, skipfooter=1)
                    ...
                    Leads_Sheets = pd.ExcelFile(output_path+'/'+wp_path+'/'+_Leads[0]) 
                except:
                    TB_NotFound = True
                ...
                try:
                    Leads_D1 = pd.read_excel(output_path+'/'+wp_path+'/'+_Leads[0], sheet_name='D1', header=header_no, skipfooter=1)
                ...
                try:
                    Leads_T1 = pd.read_excel(output_path+'/'+wp_path+'/'+_Leads[0], sheet_name='T1', header=header_no, skipfooter=1)
                ...
                try:
                    Leads_ShareCap = pd.read_excel(output_path+'/'+wp_path+'/'+_Leads[0], sheet_name='Share_Capital_Rec', header=header_no)
                except:
                ...
                try:
                    if (abs(Leads_ShareCap['Difference in Shares'])>1).any():
                        ...
                        xl.ActiveWindow.Zoom = 70
                        lastrow = workbook.Sheets('Share_Capital_Rec').UsedRange.Rows.Count
                        workbook.Sheets('Share_Capital_Rec').Cells(lastrow,1).Select()
                        ...
                        pyautogui.hotkey('right','ctrl', 'down','ctrl', 'down','ctrl', 'down','ctrl', 'shift','end')
                ...
                try:
                    Leads_AccountRec = pd.read_excel(output_path+'/'+wp_path+'/'+_Leads[0], sheet_name='Account_Rec', header=header_no)
                    Leads_BS_FS = pd.read_excel(output_path+'/'+wp_path+'/'+_Leads[0], sheet_name='Balance_Sheet_FS', header=header_no)
                ...
                try:
                    Leads_SOC = pd.read_excel(output_path+'/'+wp_path+'/'+_Leads[0], sheet_name='Statement_of_Changes', header=header_no, skipfooter=1)
            ...
            if len(_CapAct)!=0:
                ...
                try:
                    CapAct_Summary = pd.read_excel(output_path+'/'+wp_path+'/'+_CapAct[0], sheet_name='Summary', header=header_no, skipfooter=1)
                    ...
                    SC_T1 = pd.read_excel(output_path+'/'+wp_path+'/'+_CapAct[0], sheet_name='Share_Capital_T1', header=header_no+3)
                    CA_Detail = pd.read_excel(output_path+'/'+wp_path+'/'+_CapAct[0], sheet_name='Capital Activity', header=header_no)
                ...
                try:
                    CapAct_T1 = pd.read_excel(output_path+'/'+wp_path+'/'+_CapAct[0], sheet_name='Share_Capital_T1', header=header_no, skipfooter=1)
                ...
                if abs(NAVdiff)>1 or abs(PLdiff)>1:
                    ...
                    #in case it is in middle of woorkbook
                    pyautogui.press('down',presses=51)
                    time.sleep(1)
                    T1Diff = pyautogui.screenshot('T1Diff.png',region=(434,769,515,210.5))#Edited region=(400,450, 600, 520))
                ...
                try:
                    CapAct_Detail = pd.read_excel(output_path+'/'+wp_path+'/'+_CapAct[0], sheet_name='Capital Activity', header=header_no)
                except:
            ...
            if len(_CashRecon)!=0:
                ...
                xl.WindowState = client.constants.xlMaximized
                xl.FormulaBarHeight = 2
                workbook = xl.Workbooks.Open(output_path+'/'+wp_path+'/'+_CashRecon[0])
                workbook.Save()

                try:
                    Cash_Summary = pd.read_excel(output_path+'/'+wp_path+'/'+_CashRecon[0], sheet_name='Summary', header=header_no, skipfooter=1)
                    ...
                    Cash_detail = pd.read_excel(output_path+'/'+wp_path+'/'+_CashRecon[0], sheet_name='Cash_Broker_Rec', header=header_no)
                except:
                    CashNotFound = True
            ...
            if len(_Dividends)!=0:
                ...
                xl.WindowState = client.constants.xlMaximized
                xl.FormulaBarHeight = 2
                workbook = xl.Workbooks.Open(output_path+'/'+wp_path+'/'+_Dividends[0])
                workbook.Save()

                try:
                    Div_Summary = pd.read_excel(output_path+'/'+wp_path+'/'+_Dividends[0], sheet_name='Summary', header=header_no, skipfooter=1)
                    with trial: Div_Inc_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Dividends[0], sheet_name='Dividends_Income', header=header_no)
                    with trial: Div_Expense_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Dividends[0], sheet_name='Dividends_Expense', header=header_no)
                    with trial: Div_Rec_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Dividends[0], sheet_name='Dividends_Receivable', header=header_no)
                    with trial: Div_Pay_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Dividends[0], sheet_name='Dividends_Payable', header=header_no)
                ...
                if (abs(Div_Difference)>1).any() or abs(Div_taxDiff)>1:
                    ...
                    divtax_reclass_tab = []
                    div_inputs = []
                    Div_Sheets = pd.ExcelFile(output_path+'/'+wp_path+'/'+_Dividends[0])
            ...
            if len(_Interest)!=0:
                ...
                xl.WindowState = client.constants.xlMaximized
                xl.FormulaBarHeight = 2
                workbook = xl.Workbooks.Open(output_path+'/'+wp_path+'/'+_Interest[0])
                workbook.Save()

                try:
                    Int_Summary = pd.read_excel(output_path+'/'+wp_path+'/'+_Interest[0], sheet_name='Summary', header=25, skipfooter=1)
                    with trial: Int_Inc_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Interest[0], sheet_name='Interest_Income', header=header_no)
                    with trial: Int_Expense_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Interest[0], sheet_name='Interest_Expense', header=header_no)
                    with trial: Int_Rec_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Interest[0], sheet_name='Interest_Receivable', header=header_no)
                    with trial: Int_Pay_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Interest[0], sheet_name='Interest_Payable', header=header_no)
                ...
                if (abs(Int_Difference)>1).any() or abs(Int_taxDiff)>1:
                    ...
                    inttax_reclass_tab = []
                    int_inputs = []
                    Int_Sheets = pd.ExcelFile(output_path+'/'+wp_path+'/'+_Interest[0])
            ...
            if len(_FXReason)!=0:
                ...
                xl.WindowState = client.constants.xlMaximized
                xl.FormulaBarHeight = 2
                workbook = xl.Workbooks.Open(output_path+'/'+wp_path+'/'+_FXReason[0])
                workbook.Save()

                try:
                    FX_Summary = pd.read_excel(output_path+'/'+wp_path+'/'+_FXReason[0], sheet_name='FX_Reasonability', header=header_no+4)
            ...
            if len(_InvRF)!=0:
                ...
                xl.WindowState = client.constants.xlMaximized
                xl.FormulaBarHeight = 2
                workbook = xl.Workbooks.Open(output_path+'/'+wp_path+'/'+_InvRF[0])
                workbook.Save()

                try:
                    IRF_Summary = pd.read_excel(output_path+'/'+wp_path+'/'+_InvRF[0], sheet_name='Summary', header=header_no, skipfooter=1)
                    ...
                    IRF_Rollforward = pd.read_excel(output_path+'/'+wp_path+'/'+_InvRF[0], sheet_name='Roll_Forward', header=header_no+1)
                    #quantity testing:
            ...
            #quantity and cost testing    
                if (abs(IRF_quantity)>1).any() or (abs(IRF_cost)>1).any():
                    ...
                    Pivot = pd.read_excel(output_path+'/'+wp_path+'/'+_InvRF[0], sheet_name='BCR Pivot', header=9)
                    ...
                    time.sleep(1)
                    QuantityNotZero = pyautogui.screenshot('QuantityNotZero.png',region=(24,430,815,quantity_height))#Edited egion=(10,305, 1700, 600))
                    ...
                    pyautogui.press('down',presses=21)
                    time.sleep(1)
                    CostNotZero = pyautogui.screenshot('CostNotZero.png',region=(833,427,952,cost_height))#Edited region=(5,305, 1700, 600))
            ...
            if len(_PortVal)!=0:
                ...
                xl.WindowState = client.constants.xlMaximized
                xl.FormulaBarHeight = 2
                workbook = xl.Workbooks.Open(output_path+'/'+wp_path+'/'+_PortVal[0])
                workbook.Save()

                ##Tie to D1 testing
                try:
                    PortVal_TietoD1 = pd.read_excel(output_path+'/'+wp_path+'/'+_PortVal[0], sheet_name='Tie_To_D1', header=header_no+1, skipfooter=1)
                except:
                ...
                if (abs(PortVal_TietoD1['Difference'].fillna(0))>1).any():
                    ...
                    time.sleep(1)
                    PortNotZero = pyautogui.screenshot('PortNotZero.png',region=(50,460,1100, Port_height))
                    PortNotZero = add_border(PortNotZero,'PortNotZero.png')
                    if (abs(PortVal_TietoD1['Difference'].fillna(0).sum())<1):
                        reco_tietod1 = 'This is a mapping issue considering that the totals have net to nil. As such, the audit team may opt to manually reclassify the mapping of the instruments or they may update the account/instrument mapping file and ask  team for a refreshed output.'
                    else:
                ...
                try:
                    PortVal_Pricing = pd.read_excel(output_path+'/'+wp_path+'/'+_PortVal[0], sheet_name='Pricing_Summary', header=22)
                ...
                if abs(UnpricedItems)>0 or abs(pricedDifference)>=.10:
                    ...
                    if doc_fr['External Valuations'][row_num]=='N':
                        sec3 = 'No external pricing was pulled from the ISP portal for the portfolio valuations workpaper due to "External Valuations" in the  request tagged as "N".'
                        port_ss = [('PricingNotZero.png',Inches(2.5))]
                        port_memo = 'Audit team should confirm if they do not really need the pricing. Should they need the valuation, they will need to request a rerun of the Portfolio Valuations to the  delivery team.'
                    #No Priced Items
                ...
                try:  
                    Portfolio_tab = pd.read_excel(output_path+'/'+wp_path+'/'+_PortVal[0], sheet_name='Portfolio', header=header_no)
            ...
            if len(_RGL)!=0:
                ...
                xl.WindowState = client.constants.xlMaximized
                xl.FormulaBarHeight = 2
                workbook = xl.Workbooks.Open(output_path+'/'+wp_path+'/'+_RGL[0])
                workbook.Save()

                try:
                    RGL_Summary = pd.read_excel(output_path+'/'+wp_path+'/'+_RGL[0], sheet_name='Summary', header=header_no+1, skipfooter=1)
                    RGL_Detail = pd.read_excel(output_path+'/'+wp_path+'/'+_RGL[0], sheet_name='Realized_Gain_Loss', header=header_no)
                ...
                #RGL Recalculation
                try:
                    RGL_tab = pd.read_excel(output_path+'/'+wp_path+'/'+_RGL[0], sheet_name='Realized_Gain_Loss', header=header_no)
                ...
                if (abs(RGL_basereal)>1).any() or (abs(RGL_diffinreal)>1).any():
                    ...
                    RGLNotZero = pyautogui.screenshot('RGLNotZero.png',region=(191,420,892,400))#Edited egion=(10,305, 1700, 600))
                ...
                if(RGL_Detail[(RGL_Detail['Instrument Sub Class'].str.lower()=='missing')]['Fund ID'].notnull()).any() or (RGL_Detail[(RGL_Detail['Transaction Type'].str.lower()=='missing')]['Fund ID'].notnull()).any():
                    ...
                    memo_final(memo_text ='There are transactions with missing instrument/transaction mapping in the RGL workpaper.',screenshot = [('RGLMissingInsTran.png',Inches(6))],reco_text=' Delivery Team should inquire this to audit team and rerun.')
                else:
                    pass
            ...
            if len(_Trades)!=0:
                ...
                xl.WindowState = client.constants.xlMaximized
                xl.FormulaBarHeight = 2
                workbook = xl.Workbooks.Open(output_path+'/'+wp_path+'/'+_Trades[0])
                workbook.Save()

                try:
                    UT_Summary = pd.read_excel(output_path+'/'+wp_path+'/'+_Trades[0], sheet_name='Summary', header=header_no+8, skipfooter=1)
                    with trial: UT_Rec_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Trades[0], sheet_name='Trades_Receivable', header=header_no)
                    with trial: UT_Pay_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Trades[0], sheet_name='Trades_Payable', header=header_no)
                    with trial: Cap_Rec_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Trades[0], sheet_name='Capital_Receivable', header=header_no)
                    with trial: Cap_Pay_Tab = pd.read_excel(output_path+'/'+wp_path+'/'+_Trades[0], sheet_name='Capital_Payable', header=header_no)
                ...
                if (abs(Unsettled_Difference)>1).any() or abs(Unsettled_CapDiff)>1:
                    ...
                    cappay_reclass_tab = []
                    unsettled_inputs = []
                    UT_Sheets = pd.ExcelFile(output_path+'/'+wp_path+'/'+_Trades[0])
            ...
            if len(_NAVbased)!=0:
                try:
                    ...
                    workbook = xl.Workbooks.Open(output_path+'/'+wp_path+'/'+_NAVbased[0])
                ...
                try:
                    NAV_shares = pd.read_excel(output_path+'/'+wp_path+'/'+_NAVbased[0], sheet_name ='Fund_Level_Basic')
        ...
        #         docMemo.tables[1].cell(3,2).add_paragraph().add_run().add_text('No issues noted.')
            ...
            if len(req_dividends)!=0:
                if len(_Dividends)!=0:
                    Dividends_Sheets = pd.ExcelFile(output_path+'/'+wp_path+'/'+_Dividends[0])
            ...
            if len(req_interest)!=0:
                if len(_Interest)!=0:
                    Interest_Sheets = pd.ExcelFile(output_path+'/'+wp_path+'/'+_Interest[0])
            ...
            if len(missing_files)!=0:
                if len(missing_tab)!=0 and len(missing_wp)==0:
                    ...
                    sec2a = 'No files received or no transactions noted for:\n- %s\n\nAs such, the  %s working papers will not be produced. Also, the %s balances in the affected working papers will be nil.'%(missing_files_str, missing_wp_str, missing_tab_str)
                    ...
                    sec2a = 'No files received or no transactions noted for:\n- %s\n\nAs such, the  %s working papers will not be produced.'%(missing_files_str,missing_wp_str)
                    sec3b = 'The audit team shall confirm whether there are really no transactions regarding the above accounts or whether the files are really missing.'
            ...
            ##save the transformation tracker memo
            os.chdir(output_path)
            finalMemo = 'FY%s %s (%s)  Delivery Memo P%s.docx' %(dateString,doc_fr['FundName'][row_num], doc_fr['FundShortCode'][row_num],period)
            ...
            print("(FR Row Number " + str(row_num) + ") Delivery memo successfully generated.")
            time.sleep(5)
